...
class App(QWidget):
    ...
    def left_Layout(self):
        #logo
        self.logo = QLabel()
        pixmap = QPixmap(os.path.join(cwd,r"Logo.png"))
        self.logo.setPixmap(pixmap.scaledToHeight(60, Qt.SmoothTransformation))
        ...
        file_row.addWidget(self.DetectFile_btn)

        folder_row = QHBoxLayout()
        folder_row.addWidget(self.InputFolder_btn)
        folder_row.addWidget(self.DetectFolder_btn)
        
        self.left_layout.addLayout(file_row)
        self.left_layout.addLayout(folder_row)

        self.left_layout.addWidget(self.Stop_btn)
        self.left_layout.addSpacing(10)
        self.left_layout.addWidget(QLabel("➤ Log"))
        self.left_layout.addWidget(self.log)
        self.left_layout.addSpacing(10)
        self.left_layout.addWidget(QLabel("➤ Error"))
        self.left_layout.addWidget(self.error_log)
        
        self.left_layout.insertWidget(0, self.logo)
    ...
    def right_Layout(self):
        ...
        self.right_layout_info.setText("\nFolder:\n" + self.folder_path + "\n\n\nFile Excel:\n" + self.excel_path)
    ...
    def excel_input_Layout(self):
        ...
        self.input_mssv_column_title.setPlaceholderText("Tên cột MSSV (Mặc định: MSSV)")
        self.input_score_column_title.setPlaceholderText("Tên cột Điểm (Mặc định: Điểm)")
    ...
    def file_detect(self):
        if self.check_excel_file() == False:
            self.error_log.append("Tên cột (MSSV hoặc Điểm) / File Excel không hợp lệ")
            return
        ...
        if ext in image_extensions:
            ...
            self.detection_worker.finished.connect(lambda: self.DetectFile_btn.setEnabled(False))
            self.detection_worker.finished.connect(lambda: self.DetectFolder_btn.setEnabled(True))
    ...
    def folder_detect(self):
        ...
        self.log.append("Đang nhập dữ liệu...")
        self.enable_btn(False)
